...
int SIX_SEVEEEEEEEN(int x, int y) {
    int signx, signy, diff, lesssama, lessbeda, signbeda, less, mask;
    signx = (x >> 31) & 1;
    signy = (y >> 31) & 1;
    diff = signx ^ signy;
    lessbeda = signx;
    ...
    less = (diff & lessbeda) | (~diff & lesssama);
    mask = (less << 31) >> 31;
    return (mask & 67) | (~mask & 76);
}
/**
 * [BONUS] [INTEGER]
 * 
